...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:sidebar sticky collapsible="mobile" class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="__('Platform')" class="grid">
                    ...
                    <flux:sidebar.item icon="book-open-text" :href="route('posts.index')" :current="request()->routeIs('posts.*')" wire:navigate>
                        {{ __('Blog Posts') }}
                    </flux:sidebar.item>
        ...
        <x-flash-messages position="top-right" />
        ...
        <script>
            document.addEventListener('livewire:init', () => {
                Livewire.on('open-modal', (data) => {
                    const modalName = data[0]?.name || data.name;
                    if (modalName && window.$flux && window.$flux.modal) {
                        window.$flux.modal.show(modalName);
                    }
                });
